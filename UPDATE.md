...
### 🛠️ 多协议路由智能分发与解耦 (Multi-Protocol Routing & Decoupling)
- **新会话自动侦测锁死**：优化 `ConnectForm.tsx`，仅对新创建的未保存会话执行协议自动探测逻辑，防止用户编辑或切换已保存会话的 Host 字段时意外覆盖已存储的协议类型。
- **协议状态回填绑定**：解决侧边栏切换已保存会话时表单 protocol 状态未能同步绑定的问题，实现 100% 状态数据对齐。
- **配置持久化 Schema 修复**：修复了 `profileHandler.ts` 中 Profile 序列化时未保存 `protocol` 选项的漏洞，终结了重启后协议配置丢失并强制回退至默认 SSH 的严重缺陷。
- **原生本地终端 (PTY) 支持**：彻底重构本地终端的连接机制，将原先尝试通过 SSH 连接本地回路引发的 `connect ECONNREFUSED ::1:22` 报错，重构为直接在 Electron 后端拉起原生 PTY 伪终端进程 (`ptyHandler.ts`)，为极客们打通无缝的本地 shell 体验。

### 🎨 侧边栏 OS Logo 美学微重构与布局提权 (OS Logo & Layout Optimization)
- **单色高冷极客滤镜**：默认状态下对所有 OS 图标强制执行去色滤镜 (`filter saturate-0`)，完美融入黑曜石 UI 主基调。当会话被选中激活时，强制注入原厂高饱品牌色 (`saturate-100 opacity-90`) 作为视觉反馈。
- **容器限定与防缩水**：新增冷灰色直角限定边框容器 (`bg-black/20 border border-black/30 rounded-none`)，放大图标尺寸至 20px。通过强制注入 `shrink-0`，确保长服务器名称被截断时，系统图标依然坚挺、绝不挤压变形。
- **侧边栏宽度提权**：将左侧主机列表面板宽度增加 25%，为高长的主机名称提供更宽广的展示空间，大幅降低了溢出截断率。

### 🌐 国际化补充与关于面板完善 (i18n & About Updates)
- **终端设置全量汉化**：完成了对“设置-终端”配置页面的全量中文国际化翻译，消除英文残留。
- **关于面板版本注入**：在设置中的关于面板补充了丢失的 `v1.3.1` 语义化版本号，与构建码 `K9V2X` 共同构成完整的防篡改追踪标签。
